...
## How do I hand in these problems for the 11:59 a.m. ET, November 4th deadline?

Your complete .Rmd file that you create for this problem set AND the resulting .pdf (i.e., the one you 'Knit to PDF' from your .Rmd file) must be uploaded into a Quercus assignment (link: [https://q.utoronto.ca/courses/235890/assignments/733085](https://q.utoronto.ca/courses/235890/assignments/733085)) by 11:59 a.m. ET, on November 4th. Late problem sets or problems submitted another way (e.g., by email) are *not* accepted.
...
### (c) Add a new variable called `log_of_transistors` to the dataset. You can use `mutate()` and the `log()` function.
...
### (d) Plot the association between `log_of_transistors` and `year`, faceted by `unit_type` and use `geom_smooth(se=FALSE, method="lm")` to add a line of best fit to both plots. Describe this association in each plot.

Note: You will learn more about transforming variables in future courses and are not required to be able to explain why we've done this here. You can just treat `log_of_transistors` as we have other variables in class and refer to it as "the log number of transistors".
...
### (f) Calculate the correlation between `log_of_transistors` and `year` for CPUs and GPUs. You may find `group_by()`, `summarise()` and `cor()` to be helpful functions.
